...
def get_video_information_yt(file_path):
    videofiles = np.array(glob.glob(os.path.join(file_path,'v*')))
...
def smooth(d, var, winmed, winmean):
    ...
    if winmed>0:
        x = d.sort_values('frame')[var].rolling(center=True,window=winmed, min_periods=1).median()
        d[var] = x.rolling(center=True,window=winmean, min_periods=1).mean()
    else:
        d[var] = d[var]
    d['smooth'] = winmed1
    return d
...
def normalise_skeletons(df):
    ...
    df.loc[:,'Hip_angle'] = df['Hip_angle'].fillna(df.groupby('video')['Hip_angle'].transform('mean'))
    df.loc[:,'Shoulder_angle'] = df['Shoulder_angle'].fillna(df.groupby('video')['Shoulder_angle'].transform('mean'))
    #df['Hip_angle'] = df['Hip_angle'].fillna(df['Hip_angle'].mean())
    #df['Shoulder_angle'] = df['Shoulder_angle'].fillna(df['Shoulder_angle'].mean())
...
def get_dynamics_xy(xdf, delta_window):
    ...
    xdf = xdf[['video','frame','x','y','bp','fps','pixel_x', 'pixel_y','time','delta_t', 'part_idx', 'ref_dist']]
